...
# Run 2D MOEA for cost and lateness first to get appropriate functions
...
foreach(h = 1:5) %dopar% {
  ...
  bin_ship_all = c()
  for(i in 1:nrow(bin_types)){
    bin_ship_all = c(bin_ship_all,c(rep(bin_types$bin_ship[i], bin_types$bin_avail[i])))
  }
  bin_ship_all
  ...
  tau = 100
    ...
    file_name = paste0("Comb17.",h, ".txt")
    file_content = readLines(file_name)
    
    items_start = grep("200", file_content)+1
   ...
   # 
    item_due_dates = round(runif(num_items, 101, 0.6*max(bin_ship_all)*100))
    item_profits <- runif(num_items, 0.1, 0.3)*(item_area)
   # 
    item_info <- cbind(c(1:num_items),item_heights, item_widths)
    tau = 100
  # # 
  ...
  # # # Set population size
   ...
   # 
    ...
    tic()
    lp_result <- Rglpk_solve_LP(f_obj, f_con, f_dir, f_rhs, bounds = bounds, 
                                types = rep("C", length(f_obj)), 
                                max = FALSE)
    lp_toc = toc()
   # 
   ...
   N = 200
    #set.seed(2024)
     ...
     time_lateness_P0 = evaluate_lateness(solution_pop = P0$initial_pop_sol,
                                          tau = tau, due_dates = item_due_dates,
                                          bin_ship_all = bin_ship_all)
    
    
     ## Which bottom corners are in which bins
     ## and why arent all bottom coreners - oh maybe they done fit.
    
     P0_obj = cbind(c(1:N), cost_P0, time_lateness_P0, constraint_vals_P0)
    ...
    ranks = fast_non_results$Rank
     ...
     Q0 = make_new_pop(P0$initial_pop_sol, P0_rotation$initial_rotation,ranks, constraint_vals_P0,
                       total_num_bins, crowd_distance = crowding_distance)
     ...
     result_filename = paste0("Results_Combination41.",h, ".RData")
     save(num_items, numCompulsory, bin_types, all_bins, item_heights, item_widths, 
          bin_cost_all, bin_ship_all, total_num_bins, bin_heights_all, bin_widths_all,
          item_info, item_profits, item_due_dates, lp_result, tau, 
          test, file = result_filename)  # Save the test object
    
     # Store time in a text file
     time_filename = paste0("Results_Combination41.",h, ".txt")
