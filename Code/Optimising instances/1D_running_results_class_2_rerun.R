  ...
  for(letter in c("A", "B")){
    ...
    foreach(h = 0:3)  %dopar% {
      ...
      for(s in 0:9){
        ...
        file_name_c1 = paste0("R_C1_prob_", num,"_",letter,h,"_1_", s, ".RData")
        load(file_name_c1)
        this_item_due_dates = item_due_dates
        
        #all_objects <- ls()
        
        # Remove all objects except 'this_item_due_dates'
        #objects_to_remove <- setdiff(all_objects, c("this_item_due_dates", "letter", "num", "h", "s"))
        ...
        file_name <- paste0("prob_", num,"_",letter,"_",h,"_2_", s, ".txt") # Dynamically create the filename
        ...
        item_due_dates =  this_item_due_dates
        ...
        result_filename <- paste0("R_C2_prob_",num,"_", letter, h, "_2_", s, ".RData")
        save(num_items, numCompulsory, item_weights, bin_types, bin_ship, bin_avail,
             ...
             bin_capacity_all, bin_cost_all, bin_ship_all, total_num_bins, total_bins, lp_result,
             test, file = result_filename)  # Save both objects in the same file

        # Store time in a text file
        time_filename <- paste0("R_C2_prob_",num,"_", letter, h, "_2_", s, ".txt")
        writeLines(c(paste("lp_toc", lp_toc), paste("t1:", t1), paste("t2:", t2), paste("toc_end:", toc_end)), con = time_filename)
