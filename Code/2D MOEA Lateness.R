## 2D MOEA for cost and lateness objectives
## Andrea Plumbley
...
get_top_coords <- function(item_heights, item_widths, bottom_corners) {
  
  item_number <- bottom_corners[, 2]
  bottom_corner_x <- bottom_corners[, 3]
  bottom_corner_y <- bottom_corners[, 4]
  
  # Directly calculate top-right coordinates using vectorized operations
  top_right_x <- bottom_corner_x + item_widths[item_number]
  top_right_y <- bottom_corner_y + item_heights[item_number]
...
main_loop = function(Pt, Qt, Pt_rotation, Qt_rotation, m, bin_costs, numCompuslory, item_info,
  ...
  get_violation_and_corners = constraint_violation_calc(Rt, all_rotations, numCompulsory, item_info,
                                                        bin_heights, bin_widths, totalNumBins)
